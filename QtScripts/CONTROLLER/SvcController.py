...
from QtScripts.MODEL.SvcModel import SvcModel
from QtScripts.PROCESSES.SvcLearningProcess import SvcLearningProcess
from QtScripts.VIEW.SvcView import SvcView
from QtScripts.WIDGETS import input_validation as ival

logger = logging.getLogger("__SVC__")
...
class SvcController:
    def __init__(self, app: QApplication, parent_controller):
        ...
        self.model = SvcModel(self)  # set model
        ...
        self.view = SvcView(self.app, parent=self.parent_controller.view.svc_tab, controller=self)
    ...
    def load_train_dataset(self, autoload='', withwarning=True):
        if not autoload:
            dialog = self.parent_controller.parent_controller.load_path_and_update_edit(
                self.view.widgets["learn_load_train_edit"],
    ...
    @staticmethod
    def get_fixed_default_svc_params():
    ...
    def _check_params_validity(self, ):
        ...
        if ('cv' in self.view.widgets["learn_scoring_cbbox"].currentText().lower()
            or 'trust' in self.view.widgets["learn_scoring_cbbox"].currentText().lower()) and not self.view.widgets[
            "learn_kfold_ckbox"].isChecked():
            QMessageBox.critical(self.view, "Error",
                                 'You can not chose a scoring function involving Cross Validation if'
        ...
        if not self.view.widgets["learn_load_train_edit"].text():
            ...
            if not os.path.isfile(self.view.widgets["learn_load_train_edit"].text()) or not os.path.exists(
                    self.view.widgets["learn_load_train_edit"].text()):
    ...
    def _update_model_svc_params(self, ):
        local_dict = {}
        for key, value in self.view.svc_params.items():
            local_dict[key] = self.parent_controller.parent_controller.extract_widget_value(value)
        
        self.model.svc_params.update(local_dict)
    ...
    def export_metrics(self):
        ...
        if filename_:
            filename_ = filename_.replace(".svc", "_export.svc")
    ...
    def init_learning(self):
        ...
        if self._check_params_validity():
            ...
            self.view.update_svc_params_from_view()
            self.parent_controller.parent_controller.update_model_from_view(self.model, self.view)
            self._update_model_svc_params()
            self._learning_preparation()
    
    def _extract_svc_params(self,):
        svc_params = {}
        for key, widget in self.view.svc_params.items():
            if ival.isint(widget):
                svc_params[key] = int(widget)
            elif ival.isfloat(widget):
                svc_params[key] = float(widget)
            elif widget == 'None':
                svc_params[key] = None
            else:
                svc_params[key] = widget
        return svc_params
    
    def _get_param_grid_search(self, ):
        svc_params = self._extract_svc_params()
        param_grid = {}
        ...
        for k, v in svc_params.items():  # params with a single value
    ...
    def start_learning(self, combinations, n_threads, X_train, y_train, X_test, y_test):
        self.start_learning_time = datetime.datetime.now()
        self.results = {}
        ...
        for comb in combinations:
            self.params_combinations_queue.put(comb)
        
        # Add sentinel values (one per worker)
        for _ in range(n_threads):
            self.params_combinations_queue.put(None)
        
        self.learning_thread = SvcLearningProcess(self.params_combinations_queue,
                                                  X_train, y_train,
                                                  X_test, y_test,
        ...
        self.app.add_thread("svc_1", self.learning_thread)
    ...
    def _get_best_performing_combination(self, all_params_combination):
        ...
        if self.model.widgets_values["learn_scoring_cbbox"] == 'Relative K-Fold CV accuracy':
            ...
            for random_key, metrics in all_params_combination.items():
                ...
                kfold_acc_diff = round(np.abs(train_score - kcv), 3)
        ...
        if self.model.widgets_values["learn_scoring_cbbox"] == 'K-Fold CV accuracy':
            ...
            for random_key, metrics in all_params_combination.items():
                all_cv_scores = metrics[1]
                kcv = max(0.0, np.mean(all_cv_scores))
    ...
    def handle_finished(self, worker_name, ):
        ...
        for _ in range(1):
            ...
            self.view.widgets["metrics_textedit"].clear()
    ...
    def learning_display_computed_metrics(self,
        ...
        metrics_text_elements.append(
            f"Training accuracy: {display_colored_scores(all_param_combinations[best_key][2])}")
        ...
        if self.model.widgets_values["learn_kfold_ckbox"]:
            ...
            kcv_acc_diff = np.abs(round(acc - kcv, 3))
            kcv_acc_relative_diff = round(kcv_acc_diff / acc * 100, 2)
            metrics_text_elements.append(
                f"{self.model.widgets_values['learn_kfold_edit']}-fold Cross Validation: {kcv}{pm}{kcv_std}")
            metrics_text_elements.append(f"KFold-Accuracy difference = |{acc}-{kcv}| = {kcv_acc_diff}\n")
            metrics_text_elements.append(
                f"KFold-Accuracy relative difference: {display_colored_scores(kcv_acc_relative_diff, score_type='kfold')} %\n")
        ...
        metrics_text_elements.append("")
        metrics_text_elements.append("<b>SVC best performing parameters:</b>")
        ...
        if self.model.widgets_values["learn_kfold_ckbox"]:
            ...
            kfold_acc_diff = np.abs(round(mean_accuracy - mean_kfold, 3))
            ...
            metrics_text_elements.append(
                f"KFold-Accuracy relative difference: {display_colored_scores(kfold_acc_relative_diff, score_type='kfold')} %\n")
        metrics_text_elements.append("")
        
        return metrics_text_elements
    
    def reload_svc_params(self, ):
        """
        Reloads the parameters of a Random Forest Classifier and sets them to the provided string variables.

        Parameters
        ----------
        svc_params_string_var : dict
        ...
        """
        default_params = self.get_fixed_default_svc_params()
        for name, value in default_params.items():
            value = str(value)
            if value != self.view.widgets[f"params_{name}_edit"]:
                self.view.svc_params[name] = value
