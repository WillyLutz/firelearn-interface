...
class DatasetProcessingModel:
    ...
    def load_model(self, path):
        try:
            ...
            if Version(attr_dict["version"]) >= Version(params.last_version_compatible_learning):
                ...
                self.version = params.version
                QMessageBox.information(self.controller.view, "Processing Model", "Processing configuration correctly loaded."
                                                                  " \nVersion {self.version}")
                return True
            else:
                QMessageBox.warning(self.controller.view, "Error", f"You can not load a configuration version ({attr_dict['version']})"
    ...
    def save_model(self, path):
        ...
        try:
            ...
            QMessageBox.information(self.controller.view, "Info", f"Learning configuration correctly saved.\nVersion {self.version}")
        except Exception as e:
