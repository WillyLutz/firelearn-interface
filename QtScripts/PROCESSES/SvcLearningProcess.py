...
logger = logging.getLogger("__SvmLearningProcess__")


class SvcLearningProcess(QThread):
    ...
    def _learning(self, param_combination):
        ...
        for _ in range(1):
            ...
            svc.set_params(**param_combination)
            # clf_tester = ClfTester(svc)
            if self._stop_flag:
                logger.info(f"{self.objectName()} cancelled")
                break
            svc = self.train(self.X_train, self.y_train, svc)
            if self._stop_flag:
                logger.info(f"{self.objectName()} cancelled")
                break
            self.test(self.X_test, self.y_test, svc)
            ...
            if self.model_vars["learn_kfold_ckbox"]:
                ...
                cv_scores = cross_val_score(svc, X_full, y_full, cv=int(self.model_vars["learn_kfold_edit"]))
            # self.progress_queue.put(1)
            
            formatted_metrics = (param_combination, cv_scores, self.train_acc, self.test_acc, svc)
    ...
    def test_classifier(self, X, y, clf):
        ...
        for i in X.index:
            ...
            y_pred = clf.predict(np.array([row]))[0]
            proba_class = clf.predict_proba(np.array([row]))[0]
