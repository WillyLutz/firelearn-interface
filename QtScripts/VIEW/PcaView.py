...
from PyQt6.QtWidgets import QFrame, QApplication, QVBoxLayout, QHBoxLayout, QPushButton, QLineEdit, QGridLayout, QLabel, \
    QComboBox, QSizePolicy, QCheckBox, QSlider
...
from QtScripts.WIDGETS.PcaPlotTableEditor import PcaPlotTableEditor
from QtScripts.WIDGETS.TitleLabel import TitleLabel


class PcaView(QFrame):
    ...
    def manage_specific_layout(self):
        ...
        specific_ellipsis_alpha_label = QLabel("Alpha: 100", parent=self)
        specific_ellipsis_alpha_slider = QSlider(parent=self)
        specific_ellipsis_alpha_slider.setMinimum(0)
        specific_ellipsis_alpha_slider.setMaximum(100)
        specific_ellipsis_alpha_slider.setTickInterval(1)
        specific_ellipsis_alpha_slider.setOrientation(Qt.Orientation.Horizontal)
        specific_ellipsis_alpha_slider.valueChanged.connect(lambda: specific_ellipsis_alpha_label.setText("Alpha: " + str(specific_ellipsis_alpha_slider.value())))
        specific_ellipsis_alpha_slider.setValue(100)
        
        specific_components_label = QLabel("N Components:", parent=self)
        specific_components_cbbox = QComboBox(parent=self)
        specific_components_cbbox.addItems(["2", "3"])
        
        specific_ellipsis_ckbox = QCheckBox(parent=self, text="2D confidence ellipsis")
        specific_ellipsis_ckbox.setChecked(True)
        specific_show_ratio_ckbox = QCheckBox(parent=self, text="Show PC ratio")
        specific_show_ratio_ckbox.setChecked(True)
        
        specific_pca_tableplot = PcaPlotTableEditor(parent=self, combo_items=None, )
        
        # ---- Store widgets
        self.widgets["specific_load_btn"] = specific_load_btn
        self.widgets["specific_load_edit"] = specific_load_edit
        self.widgets["specific_components_cbbox"] = specific_components_cbbox
        self.widgets["specific_target_col_cbbox"] = specific_target_col_cbbox
        self.widgets["specific_pca_tableplot"] = specific_pca_tableplot
        self.widgets["specific_ellipsis_ckbox"] = specific_ellipsis_ckbox
        self.widgets["specific_ellipsis_alpha_slider"] = specific_ellipsis_alpha_slider
        self.widgets["specific_show_ratio_ckbox"] = specific_show_ratio_ckbox
        
        # ---- Connect
        specific_load_btn.clicked.connect(self.controller.load_dataset)
        (specific_target_col_cbbox.currentIndexChanged.
         connect(lambda: self.controller.update_combotable_items("specific_pca_tableplot")))
        ...
        self.specific_params_layout.addWidget(specific_components_label, 3, 0)
        self.specific_params_layout.addWidget(specific_components_cbbox, 3, 1)
        self.specific_params_layout.addWidget(specific_ellipsis_ckbox, 4, 0, 1, 2)
        self.specific_params_layout.addWidget(specific_ellipsis_alpha_label, 5, 0)
        self.specific_params_layout.addWidget(specific_ellipsis_alpha_slider, 5, 1)
        self.specific_params_layout.addWidget(specific_show_ratio_ckbox, 6, 0, 1, 2)
        
        self.specific_params_layout.addWidget(specific_pca_tableplot, 7, 0, 1, 2)
